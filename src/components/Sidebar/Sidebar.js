    ...
    const getConversations = async () => {
      try {
        ...
        const activeConversations = await response.filter((element) => {
          return (
            (element.members[0] === props.id ||
              element.members[1] === props.id) &&
            element.membersObj.length >= 2
          );
        ...
        const conversationUpdate = activeConversations.map((element) => {
          return Object.assign(element, agreeProperty);
        });
    ...
    <div className="sidebar">
      ...
      <div className="sidebar__search">
        <div className="sidebar__searchContainer">
          <SearchOutlined />
          <input placeholder="Buscar un chat activo" type="text" />
        </div>
      </div>
        ...
        {conversationNotify.length
          ? conversationNotify.map((element, index) => {
              ...
              return (
                <SidebarChat
                  key={index}
                  name={element.membersObj[selector].username}
                  photo={element.membersObj[selector].photoUrl}
                  conversation={element}
                  selectConversation={props.selectConversation}
                  selector={selector}
                  notification={element.notifications}
                />
              );
            })
          : null}
